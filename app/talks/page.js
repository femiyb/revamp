    ...
    async function fetchTalks() {
      try {
        const res = await fetch(
          'https://sessionize.com/api/speaker/json/jydf7o6y9h'
        );
        ...
        if (data?.sessions && data?.events) {
          const mappedTalks = data.sessions.map((session, index) => {
            ...
            return {
              ...
              date: matchingEvent
                ? new Date(matchingEvent.eventStartDate).toLocaleDateString(
                    undefined,
                    {
                      year: 'numeric',
                      month: 'long',
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-16 bg-gray-50 min-h-screen">
        <div className="space-y-10 mt-12">
          {talks.length > 0 ? (
            ...
            <p className="text-gray-500 text-center text-lg">
              No upcoming or past talks available.
            </p>
          )}
        </div>
        ...
        <div className="text-center mt-16">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto mb-6">
            Whether it's WordPress, accessibility, or diversity, I'm available
            to share insights with your audience. Reach out to invite me as a
            speaker.
          </p>
